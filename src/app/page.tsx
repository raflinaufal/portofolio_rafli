...
export default function HomePage() {
  ...
  const { data, loading, error } = useSelector(
    (state: RootState) => state.homeContent
  );
  const {
    data: projects,
                  ...
                  <CarouselItem key={project.id} className="md:basis-1/3">
                    <Card className="shadow-xl border-2 border-muted bg-card/80 dark:bg-card/60 flex flex-col h-full">
                      <CardHeader className="pb-2">
                        ...
                        <CardDescription className="text-base text-muted-foreground mb-2 line-clamp-2">
                          {project.description}
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="flex-1 flex flex-col">
                        ...
                        <div className="flex gap-2 flex-wrap mb-4">
                          {project.technologies?.map((tech: string) => (
                            <span
                              key={tech}
                              className="px-2 py-1 rounded bg-primary/10 text-primary text-xs font-medium border border-primary/20 shadow-sm"
                            >
                              {tech}
                            </span>
                          ))}
